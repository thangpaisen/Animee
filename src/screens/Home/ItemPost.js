...
const ItemPost = ({ item }) => {
    ...
    const blockUser = () => {
        firestore()
            ...
            .update({
                block: firestore.FieldValue.arrayUnion(userItemPost.uid)
            })
            .then(() => {
        ...
        firestore()
            ...
            .doc(auth().currentUser.uid)
            .update({
                follower: firestore.FieldValue.arrayRemove(userItemPost.uid),
                follow: firestore.FieldValue.arrayRemove(userItemPost.uid)
            })
        firestore()
            .collection('users')
            .doc(userItemPost.uid)
            .update({
    ...
    const handleClickBlockUser = () => {
        Alert.alert('Thông báo', 'Bạn muốn chặn người dùng', [
            {
                text: 'Cancel',
                onPress: () => setModalVisible(false),
                style: 'cancel'
        ...
        <>
            <View style={styles.itemPost}>
                <View style={styles.headerItemPost}>
                    ...
                    <Pressable style={styles.morePost} onPress={() => setModalVisible(true)}>
                        <Icon name='ellipsis-horizontal' size={24} color='black' />
                    </Pressable>
                </View>
                <View style={styles.content}>
                    {item?.message?.text.length > 0 && (
                        <Text style={[styles.textContent]}>{item?.message?.text}</Text>
                    )}
                    {item?.message?.image ? (
                        <Lightbox
                ...
                <View style={styles.react}>
                    <TouchableOpacity style={styles.itemIcon} onPress={() => handleOnLove()}>
                        <Icon
                    ...
                    <TouchableOpacity style={styles.itemIcon} onPress={() => handleOpenComments()}>
