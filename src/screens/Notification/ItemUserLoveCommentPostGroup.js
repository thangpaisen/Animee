...
const ItemUserLoveCommentPostGroup = ({item,handleClickButtonDelete}) => {
  ...
  useEffect(() => {
    firestore()
      ...
      .get()
      .then(doc => {
        setNameGroup(doc.data().name);
      });
  }, []);
  const handleOnWatch =()=>{  
    navigation.navigate('PostDetailGroup', {
                    data: item,
                  });
        ...
        <View style={styles.content}>
            <Text style={styles.title}>
                ...
                <Text style={{fontWeight: '600', color: 'gray'}}>
                    {' '}đã yêu thích bình luận của bạn về bài viết trong{' '}
                </Text>
                {nameGroup}
            </Text>
...
export default ItemUserLoveCommentPostGroup;
